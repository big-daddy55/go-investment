 ...
 <div class="market-body">
     <div class="list-container portfolio-list-container market-list-container" style="padding: 0 3vw; flex-wrap:wrap; <?= isset($_SESSION['user']['user_no'])  ? '' : 'display:none;' ?>">
            ...
            if (!empty($invested_companies)) {
                foreach ($invested_companies as $invested_company) : ?>
                 <?php
                    ...
                    $investedproducts = $db->query("SELECT * FROM product WHERE product_id = :product_id", [
                    ...
                    foreach ($investedproducts as $investedproduct) : ?>
                     <?php
                        ...
                        $company = $db->query("SELECT * FROM company WHERE user_no = :user_no", [
                            'user_no' => $investedproduct['user_no']
                        ])->find();
                        ?>
                     <div class="list-card" id="portfolio-list-card" style="width: 70%; height: 100%;">

                         <div class="card-head" id="portfolio-card-head">
                             <div style="display: flex; flex-direction:column;">
                                 <h4 style="margin-bottom: 0;"><?= $investedproduct['product_name'] ?></h4>
                                 <?php
                                    $word = $investedproduct['product_name'];
                             ...
                             <div id="logo">
                                 <a href="/user/product?company_no=<?= $investedproduct['user_no'] ?>">
                                     <img src=<?= $investedproduct['logo'] ?> alt="">
                                 </a>
                             </div>
                         </div>
